...
(() => {
  ...
  const MAX_DIM = 2000;
  // Even a narrow slot (a portrait trainer photo) should bank a high-res
  // source so it stays crisp on hidpi and when the layout gives it more room.
  const MIN_ENCODE = 1400;
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(MIN_ENCODE, Math.round(targetW * 2)) || MAX_DIM);
